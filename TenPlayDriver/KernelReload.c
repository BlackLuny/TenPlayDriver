...
static CHAR NtosModuleName[260] = {0};
//////////////////////////////////////////////////////////////////////////
BOOL GetWindowsRootName(WCHAR *WindowsRootName)
{
	...
	ObjectNameBuffer = (WCHAR*)ExAllocatePoolWithTag(NonPagedPool,260*2,'link');
	...
	ULONG CreateOptions)
{
	...
	FileNodeName = (WCHAR*)ExAllocatePoolWithTag(NonPagedPool,260*2,'Link');
...
BOOL ImageFile(BYTE *FileBuffer,BYTE **ImageModuleBase)
{
	...
	ImageBase=ExAllocatePoolWithTag(NonPagedPool,SizeOfImage,'Link');
	if (ImageBase==NULL)
	{
		return FALSE;
	}
...
PVOID GetKernelModuleBase(PDRIVER_OBJECT DriverObject,CHAR *KernelModuleName)
{
	...
	Buffer = (WCHAR*)ExAllocatePoolWithTag(NonPagedPool,260*2,'Link');
	if (Buffer==NULL)
	{
		return NULL;
	}
...
BOOL PeReload(KERNEL_MODULE_INFO *pKmi,PDRIVER_OBJECT DeviceObject)
{
	...
	BYTE *ImageBase;
	IO_STATUS_BLOCK IoStatus;
	st = KernelOpenFile(pKmi->wszKernelFullPath,&hFile,0x100020,0x80,1,1,0x20);
	if (!NT_SUCCESS(st))
	{
		return FALSE;
	}
	...
	FileBuffer = (BYTE*)ExAllocatePoolWithTag(PagedPool,Length,'Link');
	if (FileBuffer == NULL)
	{
		ZwClose(hFile);
		return FALSE;
	...
	ExFreePool(FileBuffer);
	FileBuffer = NULL;
	if(!FixImportTable(ImageBase,(DWORD)pKmi->pOriginKernelBase,DeviceObject))
	{
		ExFreePool(ImageBase);
		ImageBase = NULL;
		return FALSE;
	}
	if(!FixBaseRelocTable(ImageBase,(DWORD)pKmi->pOriginKernelBase))
	{
		ExFreePool(ImageBase);
		ImageBase = NULL;
		return FALSE;
	...
	pKmi->pReloadKernelBase = (PVOID)ImageBase;
